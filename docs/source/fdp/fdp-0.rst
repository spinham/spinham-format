...
FDP - "Format's Design Principle" is a design document that describes a design
principle for the spin Hamiltonian format. 
...
It is recommended to discuss the idea of FDP in the topical issue before
proposing the formal FDP document.
...
At the earliest convenience the PR should be merged by a maintainer regardless of
the state of the discussion. New PRs can be created by the Manager to update the
FDP or by Maintainer to set it status, update format, etc.
...
then the decision about the FDP shall be made by the vote with the formula
1 tool = 1 vote. The vote is organized by a Maintainer and can be called by the 
Manager or Maintainer after the discussion becomes stale.
...
* Author: <list of author's names and optionally e-mail addresses>
